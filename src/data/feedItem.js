...
const applyStatusFilter = (query, status) => {
  switch (status) {
    case "backlog":
      return query.whereNull("assignedUserId").andWhere("isDone", false);
    case "inProgress":
      return query.whereNotNull("assignedUserId").andWhere("isDone", false);
    case "done":
...
export async function fetchFeedItems({ status, limit, start }) {
  ...
  return {
    result: records,
    cursor:
      records.length > 0 ? paginator.encode(records[records.length - 1]) : null,
  };
}

export async function countFeedItems(status) {
  const baseQuery = feedItemsTable().count({ count: "id" });

  const result = await applyStatusFilter(baseQuery, status).first();
